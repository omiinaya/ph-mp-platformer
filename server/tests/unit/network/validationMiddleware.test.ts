...
describe('validationMiddleware', () => {
  ...
  describe('validatePlayerInput', () => {
    ...
    it('should reject invalid player_input - missing sequence', () => {
      ...
      wrappedListener({ input: { left: false, right: true, up: false, down: false, jump: true } });
      expect(mockSocket.emit).toHaveBeenCalledWith('error', { message: 'Invalid input' });
    ...
    it('should reject invalid player_input - wrong sequence type', () => {
      ...
      wrappedListener({ sequence: 'abc', input: { left: false, right: true, up: false, down: false, jump: true } });
      expect(mockSocket.emit).toHaveBeenCalledWith('error', { message: 'Invalid input' });
      expect(listener).not.toHaveBeenCalled();
    });
  ...
  describe('requireRoom', () => {
    ...
    it('should block room_ events when not in a room', () => {
      ...
      wrappedListener();
      expect(mockSocket.emit).toHaveBeenCalledWith('error', { message: 'Not in a room' });
      expect(listener).not.toHaveBeenCalled();
    });
    ...
    it('should block player_input when not in a room', () => {
      ...
      wrappedListener({ sequence: 1, input: { left: false, right: true, up: false, down: false, jump: true } });
      expect(mockSocket.emit).toHaveBeenCalledWith('error', { message: 'Not in a room' });
      expect(listener).not.toHaveBeenCalled();
    });
    ...
    it('should allow player_input when in a room', () => {
      ...
      const inputData = { sequence: 1, input: { left: false, right: true, up: false, down: false, jump: true } };
